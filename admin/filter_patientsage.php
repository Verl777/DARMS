...
<?php
        if (isset($_POST['search_detail'])) {
            ...
            header("Location: filter_patientsage.php?searching=$data_searched");
        } ?>
    ...
    <table id="patients">
        ...
        <tbody>
            ...
            if (isset($_GET['searching'])) {
                $search_patientage = $_GET['searching'];
            }
            $db = mysqli_connect('localhost', 'Valerian', '#Valeriephyl254', 'darms');
            $query = "SELECT * FROM patient WHERE Date_of_birth>='1998-01-01' AND Date_of_birth LIKE '%$search_patientage%'";
