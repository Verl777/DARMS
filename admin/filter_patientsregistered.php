...
<?php
        if (isset($_POST['search_detail'])) {
            ...
            header("Location: filter_patientsregistered.php?searching=$data_searched");
        } ?>
    ...
    <table id="patients">
        ...
        <tbody>
            ...
            if (isset($_GET['searching'])) {
                $search_patientregistered = $_GET['searching'];
            }
            $db = mysqli_connect('localhost', 'Valerian', '#Valeriephyl254', 'darms');
            $query = "SELECT * FROM patient WHERE createdat >= CURDATE() && createdat < (CURDATE() + INTERVAL 1 DAY) AND createdat LIKE '%$search_patientregistered%'";
