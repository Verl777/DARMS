...
<?php
        if (isset($_POST['search_detail'])) {
            ...
            header("Location: filter_receptionists.php?searching=$data_searched");
        } ?>
<div class="article">
    <style>
        #titleoftable {
    ...
    <table id="receptionists">
        ...
        <tbody>
            ...
            $query = "SELECT * FROM users WHERE user_role='receptionist' AND user_name LIKE '%$search_receptionist%'";
            $select_all_receptionists = mysqli_query($db, $query);
            $i = 0;
            while ($row = mysqli_fetch_assoc($select_all_receptionists)) {
